...
class UmadiController extends Controller
{
    function pdfHojaRuta($umadi)
    {
        $slim = Umadi::where('id', $umadi)->with([ 'familiares'])->firstOrFail();
        $tipo = 'Umadis';
    ...
    function pdf($umadi)
    {
        ...
        if ($umadi) {
            $html = view('pdf.umadi.pdf', ['umadi' => $umadi])->render();
            $pdf = app('dompdf.wrapper');
            $pdf->loadHTML($html);
            return $pdf->stream("Umadi_{$umadi->id}.pdf");
        } else {
            return response()->json(['message' => 'Umadi no encontrado'], 404);
        }
    }
    ...
    function fotoStore(Request $request, Umadi $umadi)
    {
        $request->validate([
        ...
        $disk = 'public';
        $dir  = "umadi/{$umadi->id}/fotografias";
        ...
        $foto = \App\Models\Fotografia::create([
            'caseable_id'   => $umadi->id,
            'caseable_type' => \App\Models\Umadi::class,
...
//            }
    ...
    function docStore(Request $request, Umadi $umadi)
    {
        $payload = [
            'caseable_id'   => $umadi->id,
            'caseable_type' => Umadi::class,
            'user_id'       => $request->user()?->id,
        ...
        if ($request->hasFile('file')) {
            ...
            $path        = $file->storeAs("umadi/{$umadi->id}/documentos", $storedName, 'public');
    ...
    function legalPdf($legal)
    {
        ...
        if ($legal) {
            $html = view('pdf.umadi.informe_legal', ['informe' => $legal])->render();
            $pdf = app('dompdf.wrapper');
            $pdf->loadHTML($html);
    ...
    function legalStore(Request $request, Umadi $umadi)
    {
        $data = $request->all();
        $user = $request->user();
        $data['user_id'] = $user->id;
        $data['caseable_type'] = Umadi::class;
        $data['caseable_id']   = $umadi->id;
    ...
    function psicoPdf($psico)
    {
        ...
        if ($psico) {
            $html = view('pdf.umadi.psicologica', ['sesion' => $psico])->render();
            $pdf = app('dompdf.wrapper');
            $pdf->loadHTML($html);
    ...
    function psicoStore(Request $request, Umadi $umadi)
    {
        $data = $request->all();
        $user = $request->user();
        $data['user_id'] = $user->id;
    ...
    function show(Umadi $umadi)
    {
        $umadi->load(['familiares',
...
//        public function fotografias()    { return $this->morphMany(Fotografia::class,   'caseable'); }
        return response()->json(['umadi' => $umadi]);
    }
    public function index(Request $request)
    {
        ...
        $query = Umadi::orderByDesc('created_at')->with(['familiares']);
...
//                  ->orWhere('materno', 'like', $like);
        ...
        $items->transform(function ($c) use ($user, $now) {
            ...
            if ($meAsignado) {
                switch ($rolUsuario) {
                    case 'Psicologo':
                        $hecho = Psicologica::whereMorphedTo('caseable', Umadi::class)
                            ->where('caseable_id', $c->id)->exists();
                        $labelListo = 'Primera sesión lista';
                        break;
                    case 'Abogado':
                        $hecho = InformeLegal::whereMorphedTo('caseable', Umadi::class)
                            ->where('caseable_id', $c->id)->exists();
                        $labelListo = 'Primer informe (Legal) listo';
                        break;
                    case 'Social':
                        $hecho = Problematica::whereMorphedTo('caseable', Umadi::class)
                            ->where('caseable_id', $c->id)->exists();
    ...
    public function update(Request $request, Umadi $umadi)
    {
        $payload     = $request->all();
        $umadiData    = $payload['umadi']       ?? [];
//        $adultos     = $payload['adultos']    ?? [];
        $familiares  = $payload['familiares'] ?? [];

        $umadi = DB::transaction(function () use ($umadi, $umadiData, $familiares) {

            // Actualiza el Umadis
            $umadi->update($umadiData);
            ...
            if (!empty($adultos)) {
                foreach ($adultos as $a) {
                    if (isset($a['id']) && $a['id'] > 0) {
                        // Actualiza existente
//                        $adulto = $umadi->adultos()->where('id', $a['id'])->first();
//                        if ($adulto) {
...
//                        }
            ...
            if (!empty($familiares)) {
                foreach ($familiares as $f) {
                    if (isset($f['id']) && $f['id'] > 0) {
                        // Actualiza existente
                        $familiar = $umadi->familiares()->where('id', $f['id'])->first();
                        if ($familiar) {
    ...
    public function store(Request $request)
    {
        ...
        $umadi = DB::transaction(function () use ($umadiData, $familiares, $request, $user) {

            // Crea el Umadis
            $umadiData['fecha_registro'] = now();
            $umadiData['zona'] = $user->zona ?? 'Sin zona';
            $umadiData['area'] = $user->area ?? 'Sin área';
            $umadiData['user_id'] = $user->id;

            $umadi = Umadi::create($umadiData);

            // Crea N familiares (si llegan)
            if (!empty($familiares)) {
                $umadi->familiares()->createMany($familiares);
            }
        ...
        return response()->json(['umadi' => $umadi], 201);
    }
}

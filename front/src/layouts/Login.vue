          ...
          <q-card flat bordered class="login-card">
            <q-card-section class="q-pt-lg text-center">
              ...
              <div class="text-subtitle2 text-grey-7 brand-chip">
                Dirección de Igualdad de Oportunidades <b>(DIO)</b> ·
                Gobierno Autónomo Municipal de Oruro <b>(GAMO)</b>
              </div>
            </q-card-section>
            ...
            <q-card-section class="q-pt-none text-center">
              ...
              <div class="text-caption text-grey-6">
                © {{ year }} DIO · GAMO. Todos los derechos reservados.
              </div>
            </q-card-section>
          </q-card>
        </q-form>
...
function login () {
  ...
  proxy.$axios.post('/login', { username: username.value, password: password.value })
    .then(res => {
      ...
      localStorage.setItem('tokenDio', token)
...
.login-bg-overlay {
  ...
  /* Suave desenfoque + oscurecido para contraste; ajusta a gusto */
...
.login-wrap {
  position: relative;
  z-index: 1;
  max-width: 520px;           /* Panel más pequeño */
  margin: 0 auto;
  padding: 24px 12px;
  display: flex;
  align-items: center;
